...
#define W21_MAJOR 0
#define W21_MINOR 1
#define W21_REVISION 0
// BETA 1 for beta or 0 for release
#define _W21_BETA 0x8000000000000000

#define W21_BETA(val) ((val)?_W21_BETA:0)

#define MAJOR(val) (int)((val >> 32) & 0xFFFFULL)
#define MINOR(val) (int)((val >> 16) & 0xFFFFULL)
#define REV(val) (int)(val & 0xFFFFULL)
#define W21_VERSION(major, minor, rev) (uint64_t)(((uint64_t)major)<<32|(minor)<<16|(rev))

#define W21_BUILD_DATE_GMT3 -3
#define _W21_BUILD_DATE 202607151739
#define W21_BUILD_DATE(gmt) ( (uint64_t)((gmt >= 0) ? ((uint64_t)(uint8_t)gmt) : (((uint64_t)(gmt & 0x7F) | 0x80ULL))) << 56 ) | ( _W21_BUILD_DATE )

uint64_t w21_version();
const char *w21_version_str(uint64_t *);
uint64_t w21_build_date();
const char *w21_build_date_str(uint64_t *);
